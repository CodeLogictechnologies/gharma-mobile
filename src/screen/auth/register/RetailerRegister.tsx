...
import { useRetailerRegister } from "./hooks";

const retailerRegisterSchema = yup.object({
...
export interface RetailerRegisterFormData extends yup.InferType<
  typeof retailerRegisterSchema
> {}
...
const RetailerRegister = () => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<RetailerRegisterFormData>({
    resolver: yupResolver(retailerRegisterSchema),
    defaultValues: {
      username: "",
      first_name: "",
      middle_name: "",
  ...
  const { mutate, isPending } = useRetailerRegister();

  const onSubmit = (data: RetailerRegisterFormData) => {
    mutate(data);
  };
