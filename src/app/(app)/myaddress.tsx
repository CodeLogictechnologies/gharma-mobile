...
import DeliveryBadge from "@/screen/address/component/DeliveryBadge";
import { LABEL_COLORS, LABEL_ICONS } from "@/screen/address/const";
import {
  useCustomerAddress,
  useDeleteUserAddress,
  useGetShopAddress,
} from "@/screen/address/hooks";
import { AddressItem, useAddressStore } from "@/screen/address/store";
    ...
    <>
      <View className="flex-1 bg-white">
            ...
            addresses.map((addr) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                  className={`mb-3 rounded-md border-2 p-4 bg-white ${
                    isSelected ? "border-yellow/50" : "border-transparent"
                  }`}
                >
                    ...
                    <View className="flex-1">
                      ...
                      {isSelected && (
                        <View className="flex-row items-center gap-1 mt-2">
                          <View className="w-4 h-4 rounded-full bg-[#D7A11B] items-center justify-center">
                            <Check size={10} color="white" strokeWidth={3} />
                          </View>
                          <Text className="text-xs font-medium text-[#D7A11B]">
                            Delivering here
                          </Text>
                        </View>
                      )}
        ...
        <View className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-5 pt-4 pb-8">
          <TouchableOpacity
            className="flex-row items-center justify-center gap-2 bg-yellow py-2.5 rounded-md"
